...
const TodoItem = ({ index, todo }) => {
  ...
  const REMOVE_TODO_MUTATION = gql`mutation removeTodo ($id: Int!) {
    delete_todos(where: {id: {_eq: $id}}) {
      affected_rows
    }
  }`
  const [removeTodoMutation] = useMutation(REMOVE_TODO_MUTATION);
  ...
  const [toggleTodoMutation] = useMutation(TOGGLE_TODO_MUTATION)
